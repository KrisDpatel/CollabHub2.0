    ...
    events.forEach(event => {
      ...
      card.innerHTML = `
        <div class="event-inner-vertical">
          ...
          <button class="register-btn ${now >= registrationCloseTime ? 'disabled' : ''}" 
        data-event-id="${event._id}"
        ${now >= registrationCloseTime ? 'disabled' : ''}>
  ${now >= registrationCloseTime ? 'Registration Closed' : 'Register Now'}
</button>
        </div>
      `;
...
async function loadQuestions() {
  ...
  for (const q of questions) {
    ...
    card.innerHTML = `
      <p><strong>${q.postedBy.name}${isFaculty ? ' (F)' : ''}</strong>: ${q.content}</p>
...
async function loadAnswers(questionId) {
  ...
  for (const a of answers) {
    ...
    div.innerHTML = `<p><strong>${a.postedBy.name}${isFaculty ? ' (F)' : ''}</strong>: ${a.content}</p>`;
